...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  void _loadReminders() async {
    ...
    final futureReminders = remindersBox.values
        .where((reminder) =>
            reminder.dateTime.isAfter(tomorrow) && !reminder.isCompleted)
        .toList();

    setState(() {
      _reminders = futureReminders;
    });
  }
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      navigationBar: const CupertinoNavigationBar(
        middle: Text('Lịch dự kiến'),
      ),
      child: SafeArea(
        child: _reminders.isEmpty
            ? const Center(
                child: Text(
                  'Không có lời nhắc dự kiến.',
                  style: TextStyle(
                    fontSize: 15,
