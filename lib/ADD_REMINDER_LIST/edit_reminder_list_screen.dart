...
import '../ADD_REMINDER_LIST/icon_label_data.dart';

class EditReminderListScreen extends StatefulWidget {
  final ReminderList reminderList;

  const EditReminderListScreen({super.key, required this.reminderList});

  @override
  State<EditReminderListScreen> createState() => _EditReminderListScreenState();
}

class _EditReminderListScreenState extends State<EditReminderListScreen> {
  ...
  void initState() {
    super.initState();
    _nameController.text = widget.reminderList.name;

    // Kiểm tra và khởi tạo icon hợp lệ từ dữ liệu
    _selectedIcon =
        IconData(widget.reminderList.icon, fontFamily: 'MaterialIcons');
    _selectedColor = Color(widget.reminderList.color);
  ...
  Future<void> _saveChanges() async {
    ...
    // Kiểm tra danh sách đã tồn tại (trừ chính nó)
    final isDuplicate = _reminderListBox.values.any(
        (list) => list.name == name && list.key != widget.reminderList.key);
    ...
    if (isDuplicate) {
      final shouldReplace = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text(
              'Đã tồn tại một danh sách với tên này. Bạn có muốn tiếp tục và ghi đè không?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Hủy'),
    ...
    widget.reminderList.name = name;
    widget.reminderList.icon = _selectedIcon.codePoint;
    widget.reminderList.color = _selectedColor.value;

    await widget.reminderList.save();

    Navigator.pop(context, true); // Trả về true để màn hình trước cập nhật lại
  }
